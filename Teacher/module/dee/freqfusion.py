...
class FreqFusion(nn.Module):
    ...
    def kernel_normalizer(self, mask, kernel, scale_factor=None, hamming=1):
        ...
        mask = F.softmax(mask, dim=2, dtype=mask.dtype)  # 你的代码里这行已经写得很好了
        mask = mask.view(n, mask_channel, kernel, kernel, h, w)
        mask = mask.permute(0, 1, 4, 5, 2, 3).view(n, -1, kernel, kernel)

        # --- 添加这一行，进行类型转换 ---
        # 将 hamming (可能是 float32) 转换为和 mask (可能是 float16) 一样的类型
        mask = mask * hamming.to(mask.dtype)
            ...
            if self.comp_feat_upsample:
                if self.use_high_pass:
                    ...
                    mask_lr_lr_feat = F.interpolate(
                        carafe(mask_lr_lr_feat_lr, mask_lr_init.to(mask_lr_lr_feat_lr.dtype), self.lowpass_kernel, self.up_group, 2), size=compressed_hr_feat.shape[-2:], mode='nearest')
